...
namespace Lunar.Scripts
{
    public class Script : Component<Script>
    {
        ...
        public static void DisableScripts(uint callerId) {
            foreach(Script scriptConainer in _components)
                scriptConainer.Enabled = scriptConainer.Id == callerId;
        }
        public static void EnableScripts(uint callerId)
        {
            foreach (Script scriptContainer in _components)
                scriptContainer.Enabled = true;
        }

        public static void InitScripts() { foreach(Script scriptContainter in _components) scriptContainter._script.Init(); }
        public static void LateInitScripts() { foreach(Script scriptContainter in _components) scriptContainter._script.LateInit(); }

        public static void UpdateScripts() { foreach (Script scriptContainter in _components.Where(x => x.Enabled)) scriptContainter._script.Update(); }
        public static void LateUpdateScripts() { foreach (Script scriptContainter in _components.Where(x => x.Enabled)) scriptContainter._script.LateUpdate(); }
        public static void PostRenderUpdateScripts() { foreach (Script scriptContainter in _components.Where(x => x.Enabled)) scriptContainter._script.PostRender(); }
    }
}
