...
{
    public class Transform : ObjectIdentifier<Transform>
    {
        ...
        {
            return Collection.EntryByOwner.ContainsKey(id) ? Collection.EntryByOwner[id].FirstOrDefault() : Zero; 
        }

        public static Transform GetGlobalTransform(uint id)
        {
            Transform t = Collection.ContainsKey(id) ? _components[id] : Zero;
